...
;; --init-directory points Emacs at our read-only, byte-compiled config in
;; the Nix store so it can find init.el/early-init.el/lisp/, but nothing
;; should try to *write* there. Redirect the writable stuff (snippets,
;; caches, etc.) to a real writable directory instead.
...
(push '(background-color . "#1e1e2e") initial-frame-alist)
(push '(foreground-color . "#cdd6f4") initial-frame-alist)
